...
export const swipe = async (
	...
	try {
		...
		if (!validDirections.includes(direction)) {
			res.status(400).json({ message: "Invalid direction. Must be 'left' or 'right'." });
			return;
		}
		...
		if (targetUserId === userId) {
			res.status(400).json({ message: "You cannot swipe on your own profile." });
			return;
		}
		...
		const hasUnlimitedSwipes = user.isPremium && user.premiumFeatures?.unlimitedSwipes;
		...
		const targetUserInfo = {
			isVerified: targetUser.isPremium && targetUser.premiumFeatures?.verifiedLabel
		};
