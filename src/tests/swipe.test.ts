...
describe("Swipe Controller", () => {
  ...
  it("should return 400 if targetUserId or direction is missing", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      message: "Missing targetUserId or direction."
    });
  });
  ...
  it("should return 404 if target user is not found", async () => {
    ...
    mockedUser.findById.mockResolvedValueOnce(user as any); // eslint-disable-line @typescript-eslint/no-explicit-any
    mockedUser.findById.mockResolvedValueOnce(null);
  ...
  it("should return 400 if user has already swiped the target today", async () => {
    ...
    mockedUser.findById.mockResolvedValueOnce(user as any); // eslint-disable-line @typescript-eslint/no-explicit-any
    mockedUser.findById.mockResolvedValueOnce(targetUser as any); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  it("should return 400 if user has reached daily swipe limit", async () => {
    ...
    const swipedProfiles = Array.from({ length: MAX_DAILY_SWIPES }).map(
      (_, index) => ({
  ...
  it("should allow swipe if user is non-premium and within swipe limit", async () => {
    ...
    const swipedProfiles = Array.from({ length: MAX_DAILY_SWIPES - 1 }).map(
      (_, index) => ({
        userId: new mongoose.Types.ObjectId().toHexString(),
        direction: index % 2 === 0 ? "left" : "right",
        date: today
