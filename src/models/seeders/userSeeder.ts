...
const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/dealls-dating";
...
async function generateUserData(index: number, hashedPassword: string): Promise<UserSeedData> {
    ...
    const names = [
        ["john", "smith"], ["emma", "johnson"], ["alex", "williams"],
        ["sarah", "brown"], ["mike", "jones"], ["lisa", "garcia"],
        ["david", "miller"], ["anna", "davis"], ["james", "rodriguez"],
        ["olivia", "martinez"], ["william", "hernandez"], ["sophia", "lopez"],
        ["robert", "gonzalez"], ["isabella", "wilson"], ["michael", "anderson"],
        ["emily", "thomas"], ["daniel", "taylor"], ["ava", "moore"],
        ["joseph", "jackson"], ["mia", "martin"]
    ];
...
async function seedUsers() {
    ...
    try {
        ...
        const insertedUsers = await User.insertMany(userData, { ordered: true }) as IUser[];
